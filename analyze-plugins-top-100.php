<?php
/**
 * Analyzes top 100 plugins for blueprints
 */

$zipFile = __DIR__ . '/trunk.zip';
$outputFile = __DIR__ . '/plugins-top-100.json';
...
for ( $i = 0; $i < $zip->numFiles; $i++ ) {
	...
	if ( preg_match( '#/plugins/[^/]+/[^/]+\.json$#', $filename ) ) {
		...
		if ( isset( $data['blueprints'] ) && is_array( $data['blueprints'] ) ) {
			$hasBlueprints = count( $data['blueprints'] ) > 0;
			$plugins[] = [
				'name' => $data['name'] ?? '',
				'slug' => $data['slug'] ?? '',
				'downloaded' => $data['downloaded'] ?? 0,
				'active_installs' => $data['active_installs'] ?? 0,
				'preview_url' => $hasBlueprints ? 'https://wordpress.org/plugins/' . ( $data['slug'] ?? '' ) . '/?preview=1' : false,
			];
		}
	}
}
...
echo "Done! Processed {$processedCount} total plugins, selected top 100\n";
